...
const API_TARGET = String(process.env.E2E_API_TARGET || "https://facturacion.pearandco.es").trim();
...
async function fetchRuntimeConfigForE2E() {
...
async function bootstrapAuthInBrowser(page: Page) {
  const token = readBearerFile();
...
async function fillInputIfBlank(locator: Locator, value: string) {
